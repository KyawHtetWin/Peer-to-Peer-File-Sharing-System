...
class BTPeer:
	...
	self.maxpeers = int(maxpeers) # Maximum number of known peers
	self.serverport = int(serverport)  # Port that it listens for connection
	
    # Determine host's IP address automatically if not supplied
    if serverhost:
    ...
    # list (dictionary) of known peers
	self.peers = {}        # peerid ==> (host, port) mapping
	
    self.shutdown = False  # used to stop the main loop

    # Dictionary of msgtype (4-character strings) ==> maps to specific functions (function pointers) that handles those message
	self.handlers = {}
 
    # Use the addrouter method to register a routing function. The routing function expect the name of peer
    # as a parameter and decides which of the known peer the message should be routed to next.
	self.router = None
	...
	"""
    # Create a socket to communicate using TCP/IPv4
	s = socket.socket( socket.AF_INET, socket.SOCK_STREAM )
    # Configure to make the port resuable immediately
